  ...
  return (
    <div style={{width:size,height:size,position:"relative",margin:"0 auto"}}>
      <svg viewBox="0 0 200 230" xmlns="http://www.w3.org/2000/svg"
        ...
        {[{cx:82,cy:60,c:"#2563eb"},{cx:118,cy:60,c:"#f59e0b"}].map((e,i)=>(
          <g key={i}>
...
/>
        ...
        <rect x="63" y="126" width="74" height="44" rx="10"
          fill="rgba(37,99,235,.08)" stroke="rgba(37,99,235,.25)" strokeWidth="1"/>
        <motion.circle cx="100" cy="145" r="12"
          fill="rgba(37,99,235,.15)" stroke="#2563eb" strokeWidth="2"
        ...
        {[{x:72,y:130,c:"#2563eb"},{x:72,y:148,c:"#f59e0b"},
          {x:128,y:130,c:"#f59e0b"},{x:128,y:148,c:"#ef4444"}].map((d,i)=>(
        ...
        {[{x:56},{x:100}].map((f,i)=>(
          <rect key={i} x={f.x} y="213" width="44" height="14" rx="7"
            fill="rgba(37,99,235,.2)" stroke="url(#rg)" strokeWidth="1.5"/>
        ))}
      </svg>
      {[...Array(5)].map((_,i)=>(
        <motion.div key={i} style={{
          position:"absolute",width:"5px",height:"5px",borderRadius:"50%",
          background:i%2===0?"#2563eb":"#f59e0b",
